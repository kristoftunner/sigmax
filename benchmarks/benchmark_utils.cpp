...
{
    if (!intialized) {
        LOG_ERROR("CpuInfo not initialized, skipping toJson");
        return nlohmann::json();
    }
    return { { "vendor", VendorToString(coreInfo.vendor) },
        { "uarch", UarchToString(coreInfo.uarch) },
        { "l1iCache", CacheToJson(l1iCache) },
        { "l1dCache", CacheToJson(l1dCache) },
        { "l2Cache", CacheToJson(l2Cache) },
        { "l3Cache", CacheToJson(l3Cache) },
        { "coresPerSocket", packageInfo.core_count },
        { "pageSize", pageSize } };
}
